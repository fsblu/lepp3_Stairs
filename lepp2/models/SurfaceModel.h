...
inline Coordinate operator-(Coordinate const& surface) {
  return Coordinate(-surface.x, -surface.y, -surface.z);
}
...
public:
  ...
  friend std::ostream& operator<<(std::ostream& out, SurfaceModel const& model);
private:
  int id_;
};
...
public:
  PlaneModel(double area, Coordinate const& center);
  /**
  ...
  void set_surface_type(double amount){inclination=amount;}
