...
void StairVisualizer<PointT>::drawStairs(
  ...
  for (size_t i = 0; i < sz; ++i) {
    ...
    if (i == 3) {
      ...
      pclViz.setPointCloudRenderingProperties (pcl::visualization::PCL_VISUALIZER_COLOR, 0.580392, 0, 0.827451, "STAIRS4");
    }
    if (i == 4) {
          // pcl::visualization::PointCloudColorHandlerCustom<PointT> pink (stairs[i], 200, 18, 170);
          if(!pclViz.updatePointCloud(stairs[i], "STAIRS5"))
            pclViz.addPointCloud (stairs[i], "STAIRS5");   // Yellow
          pclViz.setPointCloudRenderingProperties (pcl::visualization::PCL_VISUALIZER_COLOR, 1, 0.843137, 0, "STAIRS5");
        }
  }  
